...
class SaleOrderServiceLine(models.Model):
    ...
    @api.depends("exit_reentry", "mobilization_cost", "profile_fees", "ajeer", "iqama_transfer_fees", "iqama_fees",
                 ...
                 "total_monthly_cost", "monthly_po_without_margin", 'invoice_period',
                 )
    def _compute_total_monthly_cost(self):
        for rec in self:
            ...
            rec.monthly_po_sar_with_vat = (float_round(rec.monthly_po_sar_without_vat * 0.15, precision_digits=2)
    ...
    def create(self, vals):
        ...
        res.order_line_id.price_unit = res.order_line_id.price_subtotal = \
            sum(res.order_line_id.sale_order_service_line_ids.mapped('yearly_employee_cost')) or res.order_line_id.product_id.lst_price
        return res
    ...
    def _onchange_candidate_type(self):
        for rec in self:
            for field_name in ['gosi', 'saudization', 'iqama_fees', 'iqama_transfer_fees', 'mobilization_cost', 'ajeer',
                               ...
                               'exit_reentry', 'invoice_period', 'monthly_po_sar_without_vat',
                               'monthly_po_sar_with_vat',
                               'monthly_po_without_margin', 'total_monthly_cost', 'yearly_employee_cost']:
                rec[field_name] = 0.0
