...
class ExportBatchXlsxAdjustments(models.AbstractModel):
    _name = 'report.hr_payslip_batch_export.export_payslip_batch_adjustments'
    ...
    def generate_xlsx_report(self, workbook, data, records):
        ...
        for payslip in self.env['hr.payslip'].search([('payslip_run_id','=',data.get('batch_id'))]):
            ...
            other_earning = sum(lines.filtered(
                lambda l: l.salary_rule_id.category_id.id in gross_categories and l.salary_rule_id.amount_select == 'adjustment')
            .mapped('total'))

            net = sum(lines.filtered(
                lambda l: l.salary_rule_id.category_id.id in net_categories and l.salary_rule_id.amount_select == 'adjustment')
            .mapped('total'))
            ...
            sheet.write(row, col+8, '0', format_center)
            sheet.write(row, col+9, '0', format_center)
            sheet.write(row, col+10, other_earning or '0', format_center)
            sheet.write(row, col+11, '0', format_center)
