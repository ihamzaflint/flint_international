...
#     cleaned_number = account_number.replace('\u00A0', '').strip()
...
class ResPartnerBankInherit(models.Model):
    ...
    #     for record in self:
    #         cleaned_acc_number = clean_account_number(record.acc_number)
    #         if record.acc_number and ' ' in cleaned_acc_number:
    #             raise ValidationError(f"Spaces are not allowed in the Account Number for this number: {cleaned_acc_number}")
    ...
    def _check_partner_name(self):
        for record in self:
            if record.acc_holder_name and not all(char.isalpha() or char.isspace() for char in record.acc_holder_name):
                raise ValidationError("Symbols or numbers are not allowed in the Partner Name.")
            if record.acc_holder_name and len(record.acc_holder_name) > 35:
                raise ValidationError("Partner Name cannot exceed 35 characters.")
